...
filt_pat = (0..100).to_a.map{|x| ((((x / 100.0)) * (x / 100.0)) * 60) + 50}
...
live_loop :drums do
  ...
  sample :tabla_tas2, amp: 1.8 if n === pat_two[3]
  sleep 0.5
end
...
live_loop :frac_lead do
  sync :drums
  i = tick
  n = chord_pat.ring[i - 1]
  use_synth :pulse
  with_fx :rlpf, cutoff: 70, res: 0.7 do
    play_chord n, pulse_width: 0.3, attack:16, decay: 16, sustain_level: 0, amp: 0.7 if i % 16 === 0
  end
end
